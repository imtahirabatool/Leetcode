...
public:
    string countOfAtoms(string formula) {
        ...
        while (cur_ind < n) {
            ...
            if (cur_char == ')') {
                ...
                unordered_map<string, int> last_counter =
                    parenthesis_stack.top();
                parenthesis_stack.pop();
                unordered_map<string, int>& target =
                    parenthesis_stack.empty() ? result_counter
                                              : parenthesis_stack.top();
            ...
            while (cur_ind < n && formula[cur_ind] != '(' &&
                   formula[cur_ind] != ')') {
   ...
   target[cur_elem] +=cur_counter_str.empty() ? 1 : stoi(cur_counter_str);
            ...
            target[cur_elem] +=
                cur_counter_str.empty() ? 1 : stoi(cur_counter_str);
        }
